package server
import("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-meridian/internal/store")
...
func(s *Server)routes(){
    s.mux.HandleFunc("GET /health",func(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-meridian"})})
    ...
    s.mux.HandleFunc("PUT /api/widgets/{id}",s.handleUpdateWidget)
    s.mux.HandleFunc("POST /api/entries",s.handleLogEntry)
